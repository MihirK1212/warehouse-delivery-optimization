...
interface DeliverySidebarProps {
  ...
  selectedDelivery: DeliveryTask;
  onSelectDelivery: (delivery: DeliveryTask) => void;
  isCollapsed: boolean;
...
export default function DeliverySidebar({ 
  ...
  const otherDeliveries = filteredDeliveries.filter(delivery => delivery.id !== selectedDelivery.id);
  ...
  return (
    <div className={`transition-all duration-300 bg-white border-r border-gray-200 flex flex-col ${
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      <p className="text-sm font-medium text-gray-900 truncate">
                        {delivery.deliveryInformation.awbId}
                      </p>
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center">
                        ...
                        <p className="text-xs text-gray-600">
                          {moment(delivery.deliveryInformation.expectedDeliveryTime).format('MMM DD, HH:mm')}
                        </p>
                      </div>
        ...
        <div className="flex-1 flex flex-col items-center pt-4 space-y-4">
          ...
          <div className="space-y-2">
            <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center">
              <span className="text-xs font-medium text-blue-800">{statusCounts.dispatched || 0}</span>
            </div>
            <div className="w-8 h-8 bg-purple-100 rounded-full flex items-center justify-center">
              <span className="text-xs font-medium text-purple-800">{statusCounts.in_progress || 0}</span>
            </div>
            <div className="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center">
              <span className="text-xs font-medium text-green-800">{statusCounts.completed || 0}</span>
            </div>
          </div>
        </div>
      )}
