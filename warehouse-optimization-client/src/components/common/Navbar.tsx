'use client';

import Link from "next/link";
import { useState } from "react";
import { usePathname } from "next/navigation";

export default function Navbar() {
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const pathname = usePathname();

  const isActive = (path: string) => pathname.startsWith(path);

  return (
    <div className="bg-gray-900">
      <div className="px-4 py-5 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8">
        <div className="relative flex items-center justify-between">
          <Link
            href="/"
            aria-label="Home"
            title="Home"
            className="inline-flex items-center"
          >
            <svg
              className="w-8 text-teal-400"
              viewBox="0 0 24 24"
              fill="currentColor"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path d="M12 2L2 7v10c0 5.55 3.84 10 9 11 5.16-1 9-5.45 9-11V7l-10-5z"/>
              <path d="M9 12l2 2 4-4" stroke="white" strokeWidth="2" fill="none"/>
            </svg>
            <span className="ml-2 text-xl font-bold tracking-wide text-gray-100">
              Warehouse Delivery
            </span>
          </Link>
          
          <ul className="items-center hidden space-x-8 lg:flex">
            <li>
              <Link
                href="/scan"
                className={`font-medium tracking-wide transition-colors duration-200 ${
                  isActive('/scan')
                    ? 'text-teal-400'
                    : 'text-gray-100 hover:text-teal-400'
                }`}
              >
                Scan Items
              </Link>
            </li>
            <li>
              <Link
                href="/manage"
                className={`font-medium tracking-wide transition-colors duration-200 ${
                  isActive('/manage')
                    ? 'text-teal-400'
                    : 'text-gray-100 hover:text-teal-400'
                }`}
              >
                Manage Warehouse
              </Link>
            </li>
            <li>
              <Link
                href="/rider"
                className={`font-medium tracking-wide transition-colors duration-200 ${
                  isActive('/rider')
                    ? 'text-teal-400'
                    : 'text-gray-100 hover:text-teal-400'
                }`}
              >
                Rider Dashboard
              </Link>
            </li>
          </ul>
          
          <div className="lg:hidden">
            <button
              aria-label="Open Menu"
              title="Open Menu"
              className="p-2 -mr-1 transition duration-200 rounded focus:outline-none focus:ring-2 focus:ring-teal-400"
              onClick={() => setIsMenuOpen(true)}
            >
              <svg className="w-5 text-white" viewBox="0 0 24 24">
                <path
                  fill="currentColor"
                  d="M23,13H1c-0.6,0-1-0.4-1-1s0.4-1,1-1h22c0.6,0,1,0.4,1,1S23.6,13,23,13z"
                />
                <path
                  fill="currentColor"
                  d="M23,6H1C0.4,6,0,5.6,0,5s0.4-1,1-1h22c0.6,0,1,0.4,1,1S23.6,6,23,6z"
                />
                <path
                  fill="currentColor"
                  d="M23,20H1c-0.6,0-1-0.4-1-1s0.4-1,1-1h22c0.6,0,1,0.4,1,1S23.6,20,23,20z"
                />
              </svg>
            </button>
            {isMenuOpen && (
              <div className="absolute top-0 left-0 w-full z-50">
                <div className="p-5 bg-white border rounded shadow-sm">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <Link
                        href="/"
                        aria-label="Home"
                        title="Home"
                        className="inline-flex items-center"
                        onClick={() => setIsMenuOpen(false)}
                      >
                        <svg
                          className="w-8 text-teal-600"
                          viewBox="0 0 24 24"
                          fill="currentColor"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path d="M12 2L2 7v10c0 5.55 3.84 10 9 11 5.16-1 9-5.45 9-11V7l-10-5z"/>
                          <path d="M9 12l2 2 4-4" stroke="white" strokeWidth="2" fill="none"/>
                        </svg>
                        <span className="ml-2 text-xl font-bold tracking-wide text-gray-800">
                          Warehouse Delivery
                        </span>
                      </Link>
                    </div>
                    <div>
                      <button
                        aria-label="Close Menu"
                        title="Close Menu"
                        className="p-2 -mt-2 -mr-2 transition duration-200 rounded hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-teal-400"
                        onClick={() => setIsMenuOpen(false)}
                      >
                        <svg className="w-5 text-gray-600" viewBox="0 0 24 24">
                          <path
                            fill="currentColor"
                            d="M19.7,4.3c-0.4-0.4-1-0.4-1.4,0L12,10.6L5.7,4.3c-0.4-0.4-1-0.4-1.4,0s-0.4,1,0,1.4l6.3,6.3l-6.3,6.3 c-0.4,0.4-0.4,1,0,1.4C4.5,19.9,4.7,20,5,20s0.5-0.1,0.7-0.3l6.3-6.3l6.3,6.3c0.2,0.2,0.5,0.3,0.7,0.3s0.5-0.1,0.7-0.3 c0.4-0.4,0.4-1,0-1.4L13.4,12l6.3-6.3C20.1,5.3,20.1,4.7,19.7,4.3z"
                          />
                        </svg>
                      </button>
                    </div>
                  </div>
                  <nav>
                    <ul className="space-y-4">
                      <li>
                        <Link
                          href="/scan"
                          className="font-medium tracking-wide text-gray-700 transition-colors duration-200 hover:text-teal-600"
                          onClick={() => setIsMenuOpen(false)}
                        >
                          Scan Items
                        </Link>
                      </li>
                      <li>
                        <Link
                          href="/manage"
                          className="font-medium tracking-wide text-gray-700 transition-colors duration-200 hover:text-teal-600"
                          onClick={() => setIsMenuOpen(false)}
                        >
                          Manage Warehouse
                        </Link>
                      </li>
                      <li>
                        <Link
                          href="/rider"
                          className="font-medium tracking-wide text-gray-700 transition-colors duration-200 hover:text-teal-600"
                          onClick={() => setIsMenuOpen(false)}
                        >
                          Rider Dashboard
                        </Link>
                      </li>
                    </ul>
                  </nav>
                </div>
              </div>
            )}
          </div>
        </div>
      </div>
    </div>
  );
}